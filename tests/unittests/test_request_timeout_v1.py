...
class TestBackoffError(unittest.TestCase):
    ...
    def test_check_api_token_timeout_and_backoff(self, mocked_request):
        ...
        # initialize 'ActiveCampaignClientV1'
        try:
            with ActiveCampaignClientV1(config['api_url'],
                                      config['api_token'],
                                      config['user_agent'],
...
class TestRequestTimeoutValue(unittest.TestCase):
    ...
    '''
    @mock.patch('tap_activecampaign.client_v1.requests.Session.request', return_value = MockResponse("", status_code=200))
    @mock.patch('tap_activecampaign.client_v1.ActiveCampaignClientV1.check_api_token')
    def test_config_provided_request_timeout(self, mock_get, mock_request):
        ...
        client = ActiveCampaignClientV1(**config)
        client.request("GET", "dummy_path")
        
        mock_request.assert_called_with('GET', 'http://dummy_url.com/admin/api.php?api_action=dummy_path&api_output=json', stream=True, timeout=100.0, headers={'Api-Token': 'dummy_at', 'Accept': 'application/json', 'User-Agent': 'test_ua'})
    ...
    def test_default_value_request_timeout(self, mock_get, mock_request):
        ...
        mock_request.assert_called_with('GET', 'http://dummy_url.com/admin/api.php?api_action=dummy_path&api_output=json', stream=True, timeout=300, headers={'Api-Token': 'dummy_at', 'Accept': 'application/json', 'User-Agent': 'test_ua'})
    ...
    def test_config_provided_empty_request_timeout(self, mock_get, mock_request):
        ...
        mock_request.assert_called_with('GET', 'http://dummy_url.com/admin/api.php?api_action=dummy_path&api_output=json', stream=True, timeout=300.0, headers={'Api-Token': 'dummy_at', 'Accept': 'application/json', 'User-Agent': 'test_ua'})
    ...
    def test_config_provided_float_request_timeout(self, mock_get, mock_request):
        ...
        mock_request.assert_called_with('GET', 'http://dummy_url.com/admin/api.php?api_action=dummy_path&api_output=json', stream=True, timeout=100.5, headers={'Api-Token': 'dummy_at', 'Accept': 'application/json', 'User-Agent': 'test_ua'})
