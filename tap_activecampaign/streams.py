...
class ActiveCampaign:
    ...
    def get_and_transform_records(
        ...
        if not data or data is None or data == {} or data.get("result_code", 1) != 1:
            LOGGER.info(
                'No data for URL {}{}{}'.format(self.client.base_url, path, querystring)
            )  # No data results
            record_count = -9999
        else:  # has data
            ...
            for record in transformed_data:
                ...
                if hasattr(self, "extra_fields"):
                    extra_fields_dict = {
                        key: params[key] for key in self.extra_fields if key in params
                    }                    
                    record = {**extra_fields_dict, **record}
                    transformed_data[i] = record
...
class Campaign_report_open_list(ActiveCampaign):
    ...
    Reference : https://www.activecampaign.com/api/example.php?call=campaign_report_open_list
    """
    stream_name = "campaign_report_open_list"
    replication_keys = ["tstamp"]
    path = "campaign_report_open_list"
    data_key = "campaign_report_open_list_v1"
    created_timestamp = "tstamp"
    replication_method = "INCREMENTAL"
    ...
    def sync(
        ...
        **kwargs,
        ...
        # pagination: loop thru all pages of data
        # Pagination reference: https://www.activecampaign.com/api/example.php?call=campaign_report_open_list
        # Pagination: results are limited to 20 per page, so specify what page to view (default is 1).
        total_records = 0  # Initialize total
        t0 = perf_counter()
        partial_sync_data = partial(
            self.sync_data,
            catalog,
            state,
            start_date,
        ...
        max_parallel_tasks = kwargs.get("parallel_threads", None)
        # parallel processing
        if max_parallel_tasks is None or max_parallel_tasks > 0:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=max_parallel_tasks
            ) as executor:
                # Submit tasks for processing
                futures = {
                    executor.submit(partial_sync_data, campaign): campaign
                    for campaign in campaigns
                }

                # Wait for tasks to complete using as_completed
                for future in concurrent.futures.as_completed(futures):
                    camp_id = futures[future]
                    try:
                        ...
                        LOGGER.error(f"Error: processing CampaignID {camp_id}: {e}")
                        raise e
        else:
            ...
            if bookmark_field:
                ...
                total_records = sum(r[1] for r in result)

        time = perf_counter() - t0
        LOGGER.info(f">>>Total time: {time:.2f}s")
    ...
    def sync_data(
        ...
        offset = 0
        page = 1
        limit = 20  # Batch size; Number of records per API call; Max = 100
        record_count = limit  # Initialize, reset for each API call
        ...
        while (
            ...
            LOGGER.info(
                f"{page} bookmark:{max_bookmark_value} {record_count} total_records: {total_records} campaignid:{campaignid} endpoint_total: {endpoint_total}"
            )
    ...
    def transform_data(self, data):
        ...
        data_list = []
        data_dict = {}
        # For V1 Reporting case, each page is presented as a dictionary
        # where 'row_num' is the key and its corresponding value is the row data.
        data = [value for key, value in data.items() if key.isdigit()]
        if data_key in data:
            if isinstance(data[data_key], list):
                transformed_data = transform_json(data, self.stream_name, data_key)
            elif isinstance(data[data_key], dict):
